...
'''
# get rid of 2008 because it's weird
# df1[df1.year != 1]
class DemographicAnalysis:
    ...
    def split_demographics(self):
        ...
        # split demographics under each column into lists, must be in order
        ...
        for i1 in range(len(columns_to_split)):
            for i2 in range(1, int(self.df.loc[:,columns_to_split[i1]].max()+1)):
                self.df[demographics[i1][i2-1]] = np.where(self.df.loc[:,columns_to_split[i1]]==i2,1,0)
                demographic_columns.append(demographics[i1][i2-1])
                print(self.df[demographics[i1][i2-1]], "has been added to the dataframe.")
    ...
    def apply_weight_wave(self):
        self.demographic_columns = demographic_columns
        demographics_weighted_wave = []
        for i in range(len(demographic_columns)):
            self.df[demographic_columns[i]+'_weighted_wave'] = self.df[demographic_columns[i]] * self.df['weight_wave']
...
if __name__ == "__main__":
    da = DemographicAnalysis(df,demographic_columns)
    da.split_demographics() # <---------- separate coded demographics from category column, create columns for each demographic
    da.apply_weight_wave() 
# DemographicAnalysis.apply_weight_wave(self)
